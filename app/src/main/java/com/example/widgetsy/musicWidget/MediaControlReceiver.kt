...
class MediaControlReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        val controls = MediaListenerService.instance?.activeController?.transportControls
        if (controls == null) {
        ...
        when (intent.action) {
            ...
            ACTION_PLAY_PAUSE -> {
                val state = MediaListenerService.instance?.activeController?.playbackState?.state
